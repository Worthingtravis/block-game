...
/** Destroy a single cell at (row, col). Returns new board and the destroyed value (if any). */
export function applyBomb(board: Board, row: number, col: number): { board: Board; destroyed: number[] } {
  const next = board.map(r => [...r])
  const destroyed: number[] = []
  if (row >= 0 && row < BOARD_SIZE && col >= 0 && col < BOARD_SIZE && next[row][col] !== null) {
    destroyed.push(next[row][col]!)
    next[row][col] = null
  }
